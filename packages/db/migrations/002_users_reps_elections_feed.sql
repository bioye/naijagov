-- =============================================================
-- NaijaGov — Migration 002: Users, Auth, Representatives,
--             Feedback, Posts (Town Square)
-- =============================================================
...
CREATE TABLE users (
  ...
  phone           VARCHAR(15)  NOT NULL UNIQUE, -- E.164 format: +2348012345678
  display_name    TEXT,
  verification    verification_tier NOT NULL DEFAULT 'phone_verified',
  nin_hash        TEXT,                         -- SHA-256 of NIN, never raw
...
CREATE TABLE representatives (
  ...
  -- Constituency links (only one will be set depending on role)
  ...
  ward_id                  INT         REFERENCES wards(id),  -- for councillors

  photo_url                TEXT,
  bio                      TEXT,       -- ~100 words, plain text
  term_start               DATE,
  term_end                 DATE,
...
CREATE TABLE parties (
  ...
  slug        TEXT   NOT NULL UNIQUE,
  logo_url    TEXT,
  color_hex   VARCHAR(7),               -- e.g. #2B5F9E for APC
  founded     DATE,
  active      BOOLEAN NOT NULL DEFAULT true,
  created_at  TIMESTAMPTZ NOT NULL DEFAULT now()
);

-- Add FK now that parties table exists (referenced above in representatives)
ALTER TABLE representatives
  ADD CONSTRAINT fk_reps_party
...
CREATE TABLE candidates (
  ...
  rep_id          UUID        REFERENCES representatives(id), -- if incumbent running again
  ...
  occupation      TEXT,
  manifesto_url   TEXT,
  manifesto_summary JSONB,    -- AI-generated: {economy, security, education, health, ...}
  manifesto_summary_generated_at TIMESTAMPTZ,
  website         TEXT,
  twitter_handle  TEXT,
  facebook_url    TEXT,
  status          TEXT        NOT NULL DEFAULT 'declared',
                              -- declared | nominated | ballot_confirmed | withdrew | disqualified
  created_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at      TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE election_results (
  ...
  entered_by      UUID        REFERENCES users(id), -- admin who entered result
...
-- =============================================================
-- SEAT HISTORY  (Wikipedia-style lineage per seat)
-- =============================================================
CREATE TABLE seat_history (
  ...
  votes                    INT,
  percentage               NUMERIC(5,2),
  notes                    TEXT,        -- e.g. "Won on re-run", "Declared by court"
  created_at               TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE posts (
  ...
  -- Geographic scope of the post
  ward_id         INT         REFERENCES wards(id),
  lga_id          INT         REFERENCES lgas(id),
  state_id        INT         REFERENCES states(id),

  -- Optional candidate/rep tags
  candidate_id    UUID        REFERENCES candidates(id),
  rep_id          UUID        REFERENCES representatives(id),
  ...
  -- SEO: index posts with 5+ upvotes
...
-- Score for feed ranking: (upvotes * 2) + (verified_author ? 3 : 0) + recency_decay
-- Computed as a generated column for fast ordering
...
CREATE TABLE replies (
  ...
  parent_reply_id UUID        REFERENCES replies(id),  -- max 2 levels deep (enforced in app)
  user_id         UUID        NOT NULL REFERENCES users(id),
  body            TEXT        NOT NULL CHECK (char_length(body) <= 500),
  upvote_count    INT         NOT NULL DEFAULT 0,
  is_removed      BOOLEAN     NOT NULL DEFAULT false,
...
CREATE TABLE feedback (
  ...
  -- Context: what page/record the feedback is about
...
-- AI chat sessions linked to feedback
CREATE TABLE ai_chat_sessions (
  id              UUID        PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id         UUID        REFERENCES users(id),
  messages        JSONB       NOT NULL DEFAULT '[]',
  escalated       BOOLEAN     NOT NULL DEFAULT false,
  feedback_id     INT         REFERENCES feedback(id), -- set on escalation
  created_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at      TIMESTAMPTZ NOT NULL DEFAULT now()
);

-- =============================================================
-- UPDATED_AT triggers (remaining tables)
-- =============================================================
...
-- COMMENTS
-- =============================================================
COMMENT ON TABLE users IS 'Registered citizens. phone is in E.164 format. ward_id is their registered electoral ward.';
COMMENT ON TABLE representatives IS 'All elected/appointed officials. One row per person per role. Use seat_history for historical holders.';
COMMENT ON TABLE candidates IS 'People running in a specific election_race. Linked to rep if incumbent.';
COMMENT ON TABLE seat_history IS 'Wikipedia-style lineage: every person who ever held each seat, with party, votes, term dates.';
COMMENT ON TABLE posts IS 'Town Square posts. scoped to ward/lga/state. seo_indexed=true when upvotes >= 5.';
